...
class TreeRun(DecisionTreeClasser,db_redshift):
    ...
    def modify_data(self,sql_result):
        ...
        result = result.reindex(columns=['user_id', 'app_name', 'platform', 'puzzle_language', 'install_date',
       ...
       'retention_ts', 'retention_predict'])
        result = result.dropna(subset=['level_max', 'load_days', 'challenge_gids','challenge_games', 'coin_after', 'enjoy_status',  'iap_status'])
